...
class norm:
    def __init__(self, center, gazetarget, headrotvec, imsize, camparams,
    newfocal=960, newdistance=600):
        try:
    ...
    def __assertion(self):
        ...
        assert self.cameraparams.shape == (3,3), print("CamParams's Pattern Must Be3*3 Mat.")
        assert type(self.newfocal) == int or type(self.newfocal) == float, print ("Newfocal must be int or float.")
    ...
    def __ParamsCalculate(self):
        # Matrix: S, R, M=S*R, C_n, W=C_n*M*(C)^-1# Also provide gaze vec and head rotation matrix
        ...
        self.C_mat = np.array([[self.newfocal, 0, self.imsize[0]/2], [0, self.newfocal,
        self.imsize[1]/2], [0, 0, 1]])
        self.M_mat = np.dot(self.S_mat, self.R_mat)
        self.W_mat = np.dot(np.dot(self.C_mat, self.M_mat),
        np.linalg.inv(self.cameraparams))
    ...
    def GetImage(self, image):
        self.im = cv2.warpPerspective(image, self.W_mat, (int(self.imsize[0]),
        int(self.imsize[1])))
        return self.im
