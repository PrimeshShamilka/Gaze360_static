...
root = "/media/primesh/F4D0EA80D0EA4906/PROJECTS/FYP/Gaze detection/Datasets/gaze360_dataset_htrht37t43t9723kdfnJKhf_v2"
out_root = "/media/primesh/F4D0EA80D0EA4906/PROJECTS/FYP/Gaze detection/Datasets/gaze360_preprocessed_head"
...
def ImageProcessing_Gaze360():
    ...
    for i in range(total_num):
        ...
        # head = CropHeadImg(img, head_bbox[i])
        head = img # take original image as the head image

        # cv2.imwrite(os.path.join(out_root, "Image", category, "Head", f"{i+1}.jpg"),head)
        gaze2d = GazeTo2d(gaze)
        # save_name_face = os.path.join(category, "Face", f"{i+1}.jpg")
        # save_name_head = os.path.join(category, "Head", f"{i+1}.jpg")
        save_origin = os.path.join(recordings[0, recording_index[0, i]][0],"head", "%06d" % person_index[0, i], "%06d.jpg"% frame_index[0, i])
        ...
        save_str = " ".join([save_origin, save_gaze, save_gaze2d])
...
def CropHeadImg(img, head_bbox):
    ...
    bbox = np.array([head_bbox[0], head_bbox[1], head_bbox[2], head_bbox[3]])
    ...
    center = np.array([bbox_pixel[0], bbox_pixel[1]])
    length = int(max(bbox_pixel[2], bbox_pixel[3])/2)
    center[0] = max(center[0], length)
    center[1] = max(center[1], length)
    result = img[(center[1] - length) : (center[1] + length), (center[0] - length) : (center[0] + length)]
...
def CropEyeImg(img, head_bbox, cropped_bbox):
    ...
    result = img[(center[1] -size[1]) : (center[1] + size[1]), (center[0] - size[0]) : (center[0] + size[0])]
